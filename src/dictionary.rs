...
pub struct Words(Set<&'static str>);
...
impl Words {
    ...
    pub fn preserve(&mut self, words: &Words) {
        self.0 = self
            .0
            .intersection(&words.0)
            .cloned()
            .collect::<Set<&'static str>>();
    }

    // Consumes the set of words and returns a possibly smaller set of
    // words. The returned set doesn't have any of the words that are
    // also in the passed set.

    pub fn remove(&mut self, words: &Words) {
        self.0 = self
            .0
            .difference(&words.0)
            .cloned()
    ...
    // Consumes the set of words and returns a possibly larger set of
    // words. The returned set has all the words in both sets.

    pub fn union(&mut self, words: &Words) {
        self.0 = self
            .0
            .union(&words.0)
            .cloned()
...
// Defines the key type used in the GreenTable.
...
pub struct GreenTable(Map<GreenKey, Words>);

impl GreenTable {
    fn new_from_slice(s: &'static [&'static str]) -> GreenTable {
        let mut tbl: Map<GreenKey, Words> = Map::new();

        for word in s {
            for key in word.char_indices() {
                if let Some(set) = tbl.get_mut(&key) {
                    set.0.insert(word);
                } else {
                    let mut set = Words::new(&[]);
    ...
    pub fn new() -> GreenTable {
        GreenTable::new_from_slice(WORDS)
    }
...
pub type CharFreqKey = (usize, char);

#[derive(PartialEq, Debug)]
pub struct CharFreqTable(Map<CharFreqKey, Words>);

impl CharFreqTable {
    pub fn new_from_slice(s: &'static [&'static str]) -> CharFreqTable {
        let mut tbl: Map<CharFreqKey, Words> = Map::new();

        for word in s {
            for key in word.chars().sorted().dedup_with_count() {
                if let Some(set) = tbl.get_mut(&key) {
                    set.0.insert(word);
                } else {
                    let mut set = Words::new(&[]);
    ...
    pub fn new() -> CharFreqTable {
        CharFreqTable::new_from_slice(WORDS)
    }

    // Performs a look-up in the table.

    pub fn get(&self, key: &CharFreqKey) -> Option<&Words> {
        self.0.get(key)
    }
}
...
pub fn get_vocabulary() -> Words {
    let mut vocab: Set<&'static str> = Set::new();
...
#[cfg(test)]
pub fn mk_green_tbl(s: &'static [&'static str]) -> GreenTable {
    GreenTable::new_from_slice(s)
}
...
mod tests {
    ...
    #[test]
    fn test_union() {
        let mut a = Words::new(&["abcd"]);
        let b = Words::new(&["efgh"]);
        let c = Words::new(&["abcd", "efgh"]);
    ...
    #[test]
    fn test_remove() {
        let mut a = Words::new(&["abcd", "efgh"]);
        let b = Words::new(&["efgh"]);
        let c = Words::new(&["abcd"]);

        a.remove(&b);
        assert_eq!(a, c);

        let mut a = Words::new(&["abcd", "efgh"]);
        let b = Words::new(&["efgh", "ijkl"]);
        let c = Words::new(&["abcd"]);
    ...
    #[test]
    fn test_green_table() {
        let mut expected: GreenTable = GreenTable(Map::new());
        let tbl =
            GreenTable::new_from_slice(&["abbcc", "dddcc", "eeeea", "fffff"]);

        expected.0.insert((0, 'a'), Words::new(&["abbcc"]));
        expected.0.insert((0, 'd'), Words::new(&["dddcc"]));
        expected.0.insert((0, 'e'), Words::new(&["eeeea"]));
        expected.0.insert((0, 'f'), Words::new(&["fffff"]));
        expected.0.insert((1, 'b'), Words::new(&["abbcc"]));
        expected.0.insert((1, 'd'), Words::new(&["dddcc"]));
        expected.0.insert((1, 'e'), Words::new(&["eeeea"]));
        expected.0.insert((1, 'f'), Words::new(&["fffff"]));
        expected.0.insert((2, 'b'), Words::new(&["abbcc"]));
        expected.0.insert((2, 'd'), Words::new(&["dddcc"]));
        expected.0.insert((2, 'e'), Words::new(&["eeeea"]));
        expected.0.insert((2, 'f'), Words::new(&["fffff"]));
        expected.0.insert((3, 'c'), Words::new(&["abbcc", "dddcc"]));
        expected.0.insert((3, 'e'), Words::new(&["eeeea"]));
        expected.0.insert((3, 'f'), Words::new(&["fffff"]));
        expected.0.insert((4, 'c'), Words::new(&["abbcc", "dddcc"]));
        expected.0.insert((4, 'a'), Words::new(&["eeeea"]));
        expected.0.insert((4, 'f'), Words::new(&["fffff"]));
    ...
    #[test]
    fn test_freq_table() {
        let mut expected: CharFreqTable = CharFreqTable(Map::new());
        let tbl = CharFreqTable::new_from_slice(&[
            "abbcc", "dddcc", "eeeea", "fffff",
        ]);

        expected.0.insert((1, 'a'), Words::new(&["abbcc", "eeeea"]));
        expected.0.insert((2, 'b'), Words::new(&["abbcc"]));
        expected.0.insert((2, 'c'), Words::new(&["abbcc", "dddcc"]));
        expected.0.insert((3, 'd'), Words::new(&["dddcc"]));
        expected.0.insert((4, 'e'), Words::new(&["eeeea"]));
        expected.0.insert((5, 'f'), Words::new(&["fffff"]));
